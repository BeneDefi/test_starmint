import { X, Share2, Coins, AlertTriangle, ExternalLink, Trophy, Target, Zap } from 'lucide-react';
...
export function ShareOrMintModal({ 
  ...
  const { mintScore, isPending, isConfirming, isSuccess, txHash, mintError, getRarityFromScore } = useMintScore();
  const { address, isConnected } = useAccount();
  const { connect, connectors } = useConnect();

  if (!isOpen) return null;

  const { rarity, color: rarityColor } = getRarityFromScore(score);
      ...
      <div className="relative w-full max-w-md bg-gradient-to-br from-purple-900/90 to-cyan-900/90 rounded-2xl p-6 shadow-2xl border border-purple-500/30">
        ...
        <div className="text-center mb-6">
          ...
          <div 
            ...
            style={{ backgroundColor: `${rarityColor}20`, color: rarityColor, border: `1px solid ${rarityColor}` }}
          >
        ...
        {isSuccess ? (
          <div className="bg-green-500/20 border border-green-500/50 rounded-lg p-4 mb-4">
            ...
            <p className="text-green-200/80 text-sm text-center mb-3">
              Your high score has been immortalized on the Base blockchain with dynamic on-chain artwork!
            </p>
            {txHash && (
              <div className="flex flex-col gap-2">
                ...
                >
                  View Transaction on BaseScan <ExternalLink size={14} />
                </a>
                <a
                  href={`https://opensea.io/`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 text-sm text-purple-400 hover:text-purple-300 underline"
                >
                  View NFT on OpenSea <ExternalLink size={14} />
                </a>
              </div>
            )}
          </div>
          ...
          <>
            <div className="bg-gradient-to-r from-purple-500/10 to-cyan-500/10 border border-purple-500/20 rounded-lg p-4 mb-4">
              <h3 className="text-white font-semibold mb-2 text-center">Your NFT Preview</h3>
              <div className="bg-black/50 rounded-lg p-3 text-xs font-mono text-gray-300 space-y-1">
                <div className="flex justify-between">
                  <span className="text-gray-500">Score:</span>
                  <span style={{ color: rarityColor }}>{score.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-red-400">{enemiesDefeated}</span>
                </div>
                <div className="flex justify-between">
              ...
              <p className="text-xs text-purple-200/60 text-center mt-2">
                Dynamic on-chain SVG artwork generated from your score
              </p>
            </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className={`text-sm font-medium mb-1 ${
                      ...
                      {mintError.type === 'validation_error' && 'Validation Error'}
            ...
            <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-3 mb-2">
              <p className="text-xs text-purple-200/80 text-center leading-relaxed">
                {isConnected ? (
                  ...
                  <>Connect your wallet to mint your high score as an on-chain NFT on Base mainnet. Your achievement will be permanently stored with dynamic artwork generated from your score!</>
                )}
              </p>
            </div>
          </>
