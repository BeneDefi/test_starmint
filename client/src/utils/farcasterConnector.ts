...
export function customFarcasterConnector() {
  const connector = createConnector(config => {
    ...
    return {
      ...
      async connect() {
        try {
          // ATTEMPT 1: Try to get accounts from the base connector first
          const accounts = await baseConnector.getAccounts?.() ?? [];
          
          // ATTEMPT 2: If no accounts from base connector, try to get from miniapp context
          if (!accounts[0]) {
      ...
      async getAccounts() {
        try {
          // Try base connector first (with safe error handling)
          let accounts = [];
          try {
            accounts = await baseConnector.getAccounts?.() ?? [];
          } catch (connectorError) {
            // Base connector may fail in standalone mode, silently continue
            console.log('Base connector not available, using fallback');
          }
