...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========== SWAP SYSTEM ENDPOINTS ==========

  // Record a swap transaction
  app.post('/api/swap/record', [
    body('farcasterFid').isInt().withMessage('Farcaster FID is required'),
    ...
    body('usdNotional').isString().notEmpty().withMessage('USD notional is required'),
    body('feePaid').isString().notEmpty().withMessage('Fee paid is required'),
    body('pointsEarned').isInt({ min: 0 }).withMessage('Points earned must be non-negative'),
    handleValidationErrors
  ], async (req: Request, res: Response) => {
    try {
      const { farcasterFid, txHash, fromToken, toToken, amountIn, amountOut, usdNotional, feePaid, pointsEarned } = req.body;
      ...
      if (!user) {
        ...
        user = await storage.createUser({
          username: `farcaster_${farcasterFid}`,
          password: 'farcaster_swap_user',
          farcasterFid,
          displayName: `Trader ${farcasterFid}`,
        });
      }

      // Record the swap
      const swap = await storage.recordSwap(user.id, {
        txHash,
        fromToken,
        toToken,
        amountIn,
      ...
      res.json({
        success: true,
        swap,
        message: `Swap recorded successfully. Earned ${pointsEarned} points!`
      });
    } catch (error) {
  ...
  app.get('/api/swap/history/:farcasterFid', [
    param('farcasterFid').isInt().withMessage('Invalid Farcaster FID'),
    query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('Invalid limit'),
    handleValidationErrors
  ], async (req: Request, res: Response) => {
    try {
      ...
      const user = await storage.getUserByFarcasterFid?.(farcasterFid);
      if (!user) {
        return res.json({ swaps: [], total: 0 });
      }

      const swaps = await storage.getSwapHistory(user.id, limit);
  ...
  // Get swap points balance
  app.get('/api/swap/points/:farcasterFid', [
    param('farcasterFid').isInt().withMessage('Invalid Farcaster FID'),
    handleValidationErrors
  ], async (req: Request, res: Response) => {
    try {
      ...
      const points = await storage.getSwapPoints(user.id);
      ...
      res.status(500).json({ error: 'Failed to get swap points' });
    }
  });

  // Redeem points for STARMINT tokens or game credits
  app.post('/api/swap/points/redeem', [
    body('farcasterFid').isInt().withMessage('Farcaster FID is required'),
    body('pointsToRedeem').isInt({ min: 1 }).withMessage('Points to redeem must be at least 1'),
    body('redemptionType').isIn(['starmint_token', 'game_credits']).withMessage('Invalid redemption type'),
    handleValidationErrors
  ], async (req: Request, res: Response) => {
    try {
      const { farcasterFid, pointsToRedeem, redemptionType } = req.body;
      ...
      // Check if STARMINT token redemption is available
      if (redemptionType === 'starmint_token') {
        return res.status(400).json({ 
          error: 'STARMINT token not launched yet',
          message: 'STARMINT token redemption will be available after token launch. Stay tuned!'
        });
      }

      const redemption = await storage.redeemPoints(user.id, pointsToRedeem, redemptionType);
      const updatedPoints = await storage.getSwapPoints(user.id);

      res.json({
        success: true,
        redemption,
        remainingPoints: updatedPoints?.availablePoints || 0,
        message: `Successfully redeemed ${pointsToRedeem} points for ${redemptionType}`
      });
    } catch (error: any) {
  ...
  app.get('/api/swap/points/redemptions/:farcasterFid', [
    param('farcasterFid').isInt().withMessage('Invalid Farcaster FID'),
    query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('Invalid limit'),
    handleValidationErrors
  ], async (req: Request, res: Response) => {
    try {
      ...
      const redemptions = await storage.getPointsRedemptions(user.id, limit);
  ...
  // Get swap configuration and supported tokens
  app.get('/api/swap/config', (req: Request, res: Response) => {
    res.json({
      supportedTokens: [
        {
          symbol: 'ETH',
        ...
        },
        {
          symbol: 'STARMINT',
          name: 'STARMINT Token',
          address: null, // Will be set after token launch
          decimals: 18,
          isNative: false,
      ...
      feeStructure: {
        minimumSwapUsd: 0.10,
        flatFeeUsd: 0.10,
        percentageFeeThresholdUsd: 500,
        percentageFeeBps: 10 // 0.1%
      },
      pointsSystem: {
        pointsPerTenDollars: 1,
        starmintRedemptionEnabled: false, // Not launched yet
        gameCreditsRedemptionEnabled: true
      }
    });
  });
